...
from utils.mast3r_utils_heejung import Mast3rMatcher #################################################
import cv2
from scipy.optimize import least_squares
from torch.optim.lr_scheduler import ExponentialLR

# ✅ 추가: 프레임별 신뢰도 점수 로드 함수
from utils.compression_aware_utils import load_frame_trust_metrics
...
def training(dataset, opt, pipe, dataset_name, debug_from, logger=None):
    ...
    try:
        frame_trust_dict = load_frame_trust_metrics(
            ...
            keypoint_csv=None,  # keypoint 매칭 로그 생략
            gaussian_csv=os.path.join(scene.model_path, "gaussian_stats_log.csv"),
            debug=True
        )
        logger.info(f" 💙💙 [Compression-Aware] Frame trust loaded: {len(frame_trust_dict)} frames ({scene_name}, {qp_level})")
    except Exception as e:
        logger.warning(f"Frame trust not available ({scene_name}, {qp_level}) → default=1.0 ({e})")
        frame_trust_dict = {}

    print(f"[DEBUG] frame_trust_dict for {scene_name} ({qp_level}):", len(frame_trust_dict))
    ...
    #         keypoint_csv=os.path.join("/keypoint_match_log.csv"),
    #         gaussian_csv=os.path.join(scene.model_path, "gaussian_stats_log.csv"),
    #         debug=True
    #     )
    #     logger.info(f"[Compression-Aware] Frame trust loaded: {len(frame_trust_dict)} frames")
    # except Exception as e:
    #     logger.warning(f"Frame trust not available ({e}), default=1.0")
    #     frame_trust_dict = {}

    # print('frame_trust_dict:', frame_trust_dict)
        ...
        with torch.no_grad():
            ...
            if iteration < opt.update_until and iteration > opt.start_stat:
                ...
                if iteration > opt.update_from and iteration % opt.update_interval == 0:
                    # 🔁 최신 신뢰도 메트릭 다시 계산 (매 update_interval 마다)
                    frame_trust_dict = load_frame_trust_metrics(
                        qp_csv="/workdir/comp_log/grass_qp37_trustmap.csv",  # 고정
                        keypoint_csv=os.path.join(scene.model_path, "keypoint_match_log.csv"),
                        gaussian_csv=os.path.join(scene.model_path, "gaussian_stats_log.csv"),
                        loss_csv=os.path.join(scene.model_path, "render_loss_log.csv"),
                        debug=True
                    )

                    # 🔹 현재 viewpoint_cam의 프레임 ID
                    frame_id = int(viewpoint_cam.uid)

                    # 🔹 최신 frame_trust_dict에서 해당 프레임 신뢰도 가져오기 (없으면 1.0)
                    frame_trust = frame_trust_dict.get(frame_id, 1.0)
                    print(f"[Iter {iteration}] Frame ID {frame_id} → Trust {frame_trust:.3f}")

                    # 🔹 frame_trust를 adjust_anchor로 전달
                    gaussians.adjust_anchor_heejung(
                    ...
                    # gaussians.adjust_anchor_heejung(check_interval=opt.update_interval, success_threshold=opt.success_threshold, grad_threshold=opt.densify_grad_threshold, min_opacity=opt.min_opacity, require_purning = True)
            

                    #############################################
                    # [추가 코드] Gaussian 생성 정보 CSV 저장
                    #############################################
                    import csv
                    import numpy as np
    ...
    while start_view_id < num_views:
        ...
        if scene.getTrainCameras()[end_view_id-1].is_registered == False:
            ...
            with torch.no_grad():
                ...
                viewpoint_cam.kp0, viewpoint_cam.kp1, _, _, _, _, _, _, \
                    viewpoint_cam.pre_depth_map, viewpoint_cam.depth_map = \
                    matcher._forward(
                        pre_viewpoint_cam1.original_image,
                        viewpoint_cam.original_image,
                        intrinsic_np,
                        frame_trust=frame_trust  # ✅ 압축-aware 신뢰도 전달
                    )
                ...
                ####################### 추가
                inlier_ratio = len(inliers) / len(kp1_np) if inliers is not None else 0.0

                if inlier_ratio < 0.2:
                    frame_trust_dict[frame_id] *= 0.7  # 자동 감쇠
                    logger.info(f"[Low Inlier Ratio] Frame {frame_id} → trust ↓ {frame_trust_dict[frame_id]:.3f}")
            ...
            with torch.no_grad():
                ...
                if iteration < opt.update_until and iteration > opt.start_stat:
                    ...
                    if iteration > opt.update_from and iteration % opt.update_interval == 0:
                        ...
                        # gaussians.adjust_anchor_heejung(check_interval=opt.update_interval, success_threshold=opt.success_threshold, grad_threshold=opt.densify_grad_threshold, min_opacity=opt.min_opacity, require_purning = False)
            ...
            with torch.no_grad():
                ...
                if iteration < opt.update_until and iteration > opt.start_stat:
                    ...
                    if iteration > opt.update_from and iteration % opt.update_interval == 0:
                        ...
                        # gaussians.adjust_anchor_heejung(check_interval=opt.update_interval, success_threshold=opt.success_threshold, grad_threshold=opt.densify_grad_threshold, min_opacity=opt.min_opacity)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            # # 🔁 여기에 adaptive scaling 추가
            # if iteration % 1000 == 0 and iteration > opt.update_from:
            #     try:
            #         avg_loss = float(Ll1.item())
            #         if "baseline_loss" not in locals():
            #             baseline_loss = avg_loss
            #         loss_scale = np.clip(1.0 - (avg_loss - baseline_loss) * 0.5, 0.8, 1.2)
            #         for fid in frame_trust_dict.keys():
            #             frame_trust_dict[fid] = np.clip(frame_trust_dict[fid] * loss_scale, 0.3, 1.0)
            #         logger.info(f"[Adaptive Trust] Iter={iteration}, avg_loss={avg_loss:.5f}, scaled={loss_scale:.2f}")
            #     except Exception as e:
            #         logger.warning(f"[Adaptive Trust] skipped ({e})")
