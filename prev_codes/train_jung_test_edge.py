...
def training(dataset, opt, pipe, dataset_name, debug_from, logger=None):
    ...
    def train_uid_to_real_id(t: int) -> int:
        # 매 8장의 train 뒤에 test 프레임(9의 배수)이 1장씩 존재하므로 보정
        return t + 1 + (t // 8)
    # ============================================
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            if iteration < opt.update_until and iteration > opt.start_stat:
                ...
                # ============================================
                # [PSNR-aware 추가 ②] Densify/Prune 스케일링
                # ============================================

                cur_uid = int(viewpoint_cam.uid)
                cur_real = train_uid_to_real_id(cur_uid)
                cur_psnr = psnr_map.get(cur_real, 30.0)  # default 30dB (fallback)
                ...
                # PSNR 기반 조건
                if cur_psnr > 30:        # I-frame급 (고품질)
                    densify_scale = 1.0
                    require_pruning = False
                elif cur_psnr > 28:      # 중간 품질
                    densify_scale = 0.7
                    require_pruning = False
                else:                    # 저품질 (압축 artifact 심함)
                    densify_scale = 0.4
                ...
                if iteration > opt.update_from and iteration % opt.update_interval == 0:
                    ...
                    gaussians.adjust_anchor(
                        ...
                        grad_threshold=opt.densify_grad_threshold * densify_scale,
                        min_opacity=opt.min_opacity,
                        require_purning=require_pruning
                    )
