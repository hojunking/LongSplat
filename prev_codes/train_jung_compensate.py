...
def training(dataset, opt, pipe, dataset_name, debug_from, logger=None):
    ...
    # dataset_name 대신 args.source_path 기준으로 판정
    source_path_lower = args.source_path.lower()
    is_compressed = bool(re.search(r'qp\d+', source_path_lower)) or "compress" in source_path_lower

    if is_compressed:
        logger.info("🟠 Compressed input detected — compensation enabled.")
    else:
        logger.info("🔵 Non-compressed input detected — standard training applied.")

    if is_compressed:
        logger.info("🟢 Detected compressed input — applying compression-aware training adjustments.")
        
        # 🔹 1️⃣ Threshold 보정
        opt.densify_grad_threshold *= 0.8   # 가우시안 생성 민감도 완화
        opt.min_opacity *= 0.7              # 투명도 유지 완화
        opt.success_threshold *= 0.9        # pose success 기준 완화
        opt.pruning_ratio *= 0.8            # pruning 비율 축소
        logger.info(f" - Thresholds adjusted (densify={opt.densify_grad_threshold}, opacity={opt.min_opacity})")

        # 🔹 2️⃣ Opacity 초기값 보정
        try:
            if hasattr(dataset, "use_feat_bank") and hasattr(gaussians, "get_opacity"):
                opacity_vals = gaussians.get_opacity().detach().cpu()
                opacity_vals *= 1.1
                logger.info("✅ Opacity initialization boosted (x1.1).")
            else:
                logger.warning("Opacity boost skipped (no attribute).")
        except Exception as e:
            logger.warning(f"Opacity initialization skipped due to: {e}")

        # 🔹 3️⃣ 학습률 조정
        opt.translation_lr_init *= 1.2
        opt.rotation_lr_init *= 1.1
        logger.info(f" - LR adjusted: translation={opt.translation_lr_init}, rotation={opt.rotation_lr_init}")

        # 🔹 4️⃣ Soft Sharpen 필터 정의 (밝기(Y) 채널 기반)
        try:
            import cv2, numpy as np, torch

            def soft_sharpen_image(img_torch):
                """압축 영상용 Soft Sharpen — Y채널만 부드럽게 강조"""
                img_np = (img_torch.detach().cpu().numpy().transpose(1,2,0) * 255).astype(np.uint8)
                yuv = cv2.cvtColor(img_np, cv2.COLOR_RGB2YUV)
                kernel = np.array([[0, -0.5, 0],
                                   [-0.5, 3.0, -0.5],
                                   [0, -0.5, 0]])
                yuv[..., 0] = cv2.filter2D(yuv[..., 0], -1, kernel)
            ...
            # global로 접근할 수 있게 함수 등록
            globals()['sharpen_image'] = soft_sharpen_image
            logger.info(" - Soft sharpening (Y-channel) initialized.")

        except Exception as e:
            sharpen_image = lambda x: x
            logger.warning(f"Sharpen filter unavailable: {e}")
    ...
    while start_view_id < num_views:
        ...
        if scene.getTrainCameras()[end_view_id-1].is_registered == False:
            ...
            with torch.no_grad():
                ...
                # #############################################
                # # [코드 추가 2] keypoint/inlier csv 기록
                # #############################################
                # # 실제 프레임 번호 계산
                # #  - num_views: 전체 프레임 수 (train + test)
                # #  - test_frame_ids: 9의 배수 (0, 9, 18, ...)
                # test_frame_ids = set([i for i in range(0, num_views) if i % 9 == 0])

                # # scene.getTrainCameras()의 UID는 train 인덱스임 → 실제 frame_id로 매핑
                # # train 인덱스를 실제 인덱스로 복원하려면 test 프레임들을 건너뛰어야 함
                # train_to_real_map = []
                # real_idx = 0
                # for i in range(num_views):
                #     if i not in test_frame_ids:
                #         train_to_real_map.append(i)
                # real_frame_id = train_to_real_map[viewpoint_cam.uid]
                # prev_real_frame_id = train_to_real_map[pre_viewpoint_cam1.uid]

                # # keypoint와 inlier 개수 기록
