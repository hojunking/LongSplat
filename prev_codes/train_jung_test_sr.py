...
def training(dataset, opt, pipe, dataset_name, debug_from, logger=None):
    ...
    # ---- PSNR 데이터 불러오기 ----
    psnr_csv_path = "/workdir/comp_log/x265_3dgs-dataset__free_dataset__free_dataset__grass__images_qp37.csv"
    psnr_df = pd.read_csv(psnr_csv_path)
    psnr_df.columns = [c.strip().replace(" ", "_") for c in psnr_df.columns]
    psnr_map = {int(r["Encode_Order"]): float(r["Y_PSNR"]) for _, r in psnr_df.iterrows()}

    # ---- train uid → 실제 프레임 번호 ----
    def train_uid_to_real_id(t: int) -> int:
        # train uid(0..len(train)-1) → 실제 프레임 인덱스(0..N_total-1)
        # 매 8장의 train 뒤에 test(9의 배수)가 1장씩 존재 → 보정
        return t + 1 + (t // 8)

    # ---- PSNR 차이가 가장 작은 참조 프레임 선택 ----
    def select_reference_cam(scene, cur_uid: int, window: int = 4):
        cams = scene.getTrainCameras()
        cur_view = cams[cur_uid]
        cur_real = train_uid_to_real_id(cur_uid)
        cur_psnr = psnr_map.get(cur_real, None)

        best_cam = None
        best_diff = float("inf")

        # 과거 window 범위 탐색
        for cand_uid in range(max(0, cur_uid - window), cur_uid):
            cand_cam = cams[cand_uid]
            if not cand_cam.is_registered:
                continue
            cand_real = train_uid_to_real_id(cand_uid)
            cand_psnr = psnr_map.get(cand_real, None)
            if (cur_psnr is None) or (cand_psnr is None):
                continue
            diff = abs(cur_psnr - cand_psnr) # 절댓값으로 차이 계산
            if diff < best_diff:
                best_diff = diff
                best_cam = cand_cam

        # 후보가 없으면 직전 프레임 사용
        if best_cam is None and cur_uid > 0:
            best_cam = cams[cur_uid - 1]
    ...
    while start_view_id < num_views:
        ...
        if scene.getTrainCameras()[end_view_id-1].is_registered == False:
            ...
            with torch.no_grad():
                ...
                # 기존 pre_viewpoint_cam1 대신 PSNR 기반 참조 선택
                # pre_viewpoint_cam1 = select_reference_cam(scene, int(viewpoint_cam.uid), window=2)

                # ===========================================
                # (1) 항상 직전 프레임을 기본 reference로 사용
                # ===========================================
                if int(viewpoint_cam.uid) == 0:
                    continue
                pre_viewpoint_cam1 = scene.getTrainCameras()[int(viewpoint_cam.uid) - 1]
                ref_img = pre_viewpoint_cam1.original_image.clone()

                # ===========================================
                # (2) PSNR 유사 프레임 탐색 (window=2)
                # ===========================================
                window = 2
                cur_uid = int(viewpoint_cam.uid)
                cur_real = train_uid_to_real_id(cur_uid)
                cur_psnr = psnr_map.get(cur_real, None)
                ...
                for cand_uid in range(max(0, cur_uid - window), cur_uid):
                    ...
                    if cand_uid == pre_viewpoint_cam1.uid:
                        continue
                    cand_cam = scene.getTrainCameras()[cand_uid]
                    cand_real = train_uid_to_real_id(cand_uid)
                    cand_psnr = psnr_map.get(cand_real, None)
                    if cand_psnr is None or cur_psnr is None:
                        continue
                    diff = abs(cur_psnr - cand_psnr)
                    if diff < best_diff:
                        best_diff = diff
                        best_cand = cand_cam
                ...
                # (3) blending 수행 + 결과 저장
                # ===========================================
                if best_cand is not None and best_diff < 2.0:
                    blend_ratio = 0.2
                    w = np.exp(-best_diff)
                    blend_img = (1 - blend_ratio) * ref_img + blend_ratio * w * best_cand.original_image
                    print(f"[Ref Correction] Frame {cur_real:03d}: blended with frame {train_uid_to_real_id(best_cand.uid):03d}, ΔPSNR={best_diff:.2f} dB")

                    # ---------- 저장 (base/blend/diff) ----------
                    import torchvision.transforms.functional as TF
                    save_dir = os.path.join(scene.model_path, "ref_blend_debug")
                    os.makedirs(save_dir, exist_ok=True)

                    frame_str = f"{cur_real:03d}"
                    base_path = os.path.join(save_dir, f"ref_frame_{frame_str}_base.png")
                    blend_path = os.path.join(save_dir, f"ref_frame_{frame_str}_blend.png")
                    diff_path = os.path.join(save_dir, f"ref_frame_{frame_str}_diff.png")

                    base_img = ref_img.detach().cpu().clamp(0, 1)
                    blend_img_clamped = blend_img.detach().cpu().clamp(0, 1)
                    TF.to_pil_image(base_img[0]).save(base_path)
                    TF.to_pil_image(blend_img_clamped[0]).save(blend_path)



                    # ✅ diff map 생성 (항상 RGB 3채널로 제한)
                    diff_map = torch.abs(base_img - blend_img_clamped)

                    # --- shape 보정 (항상 4D: (1,3,H,W)) ---
                    if diff_map.ndim == 3:
                        diff_map = diff_map.unsqueeze(0)  # (3,H,W) → (1,3,H,W)
                    elif diff_map.ndim == 2:
                        diff_map = diff_map.unsqueeze(0).unsqueeze(0)  # (H,W) → (1,1,H,W)

                    print('✅ diff_map fixed shape:', diff_map.shape)

                    # --- grayscale 평균 + RGB 확장 ---
                    diff_map = diff_map.mean(dim=1, keepdim=True)       # (1,1,H,W)
                    diff_map = diff_map.expand(-1, 3, -1, -1).clamp(0, 1)  # (1,3,H,W)
                    TF.to_pil_image(diff_map[0]).save(diff_path)


                    TF.to_pil_image(diff_map[0]).save(diff_path)
                    print(f"[Save] Frame {frame_str}: base/blend/diff saved → {save_dir}")

                    # blended image를 reference로 교체
                    pre_viewpoint_cam1.original_image = blend_img


                voxel_visible_mask = prefilter_voxel(pre_viewpoint_cam1, gaussians, pipe, background)
                pre_render_pkg = render(pre_viewpoint_cam1, gaussians, pipe, background,
                ...
                viewpoint_cam.kp0, viewpoint_cam.kp1, _, _, _, _, _, _, viewpoint_cam.pre_depth_map, viewpoint_cam.depth_map = \
                    matcher._forward(pre_viewpoint_cam1.original_image, viewpoint_cam.original_image, intrinsic_np)
                ...
                pre_pts = unporject(pre_rendered_depth, pre_viewpoint_cam1.view_world_transform,
                                    pre_viewpoint_cam1.intrinsic, viewpoint_cam.kp0)
                ...
                # 🔹 (추가) 참조 프레임 거리 계산 (몇 프레임 전)
                frame_distance = train_uid - prev_uid  # uid 기준 거리

                # 🔹 (추가) PSNR 차이 계산 (품질 유사성)
                cur_real_id = real_frame_id
                prev_real_id = prev_real_frame_id
                cur_psnr = psnr_map.get(cur_real_id, None)
                prev_psnr = psnr_map.get(prev_real_id, None)
                if (cur_psnr is not None) and (prev_psnr is not None):
                    delta_psnr = cur_psnr - prev_psnr
                else:
                    delta_psnr = None

                # 🔹 디버그 프린트 (콘솔에서 즉시 확인)
                print(f"[Pose-Estimation] Frame {real_frame_id:03d} ← Ref {prev_real_frame_id:03d} "
                    f"({frame_distance:+d} prev), ΔPSNR={delta_psnr:.2f} dB, "
                    f"Keypoints={num_keypoints}, Inliers={num_inliers}, Success={pnp_success}")
                ...
                if not os.path.exists(keypoint_csv_path):
                    with open(keypoint_csv_path, 'w', newline='') as f:
                        ...
                        writer.writerow([
                            ...
                            "Frame_Distance", "Delta_PSNR",
                            "Num_Keypoints", "Num_Inliers", "PnP_Success"
                        ])

                with open(keypoint_csv_path, 'a', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow([
                        real_frame_id, prev_real_frame_id,
                        frame_distance, delta_psnr,
                        num_keypoints, num_inliers, pnp_success
                    ])
                #############################################
