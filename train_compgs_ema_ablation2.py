...
def training(dataset, opt, pipe, dataset_name, debug_from, logger=None):
    ...
    while start_view_id < num_views:
        ...
        for iteration in range(first_iter, opt.iterations + 1):
            ...
            # # 모듈 3 (c) Quality Gap-aware Masking 적용
            # # dropout rate 적용 ################# 
            ...
            with torch.no_grad():
                ...
                if iteration < opt.update_until and iteration > opt.start_stat:
                    ...
                    if iteration > opt.update_from and iteration % opt.update_interval == 0:
                        ...
                        # adjust_anchor_scale_pruning 적용 (모듈(b) adaptive densification만 활성화)
                        gaussians.adjust_anchor_scale_pruning_adaptive(
                            check_interval=opt.update_interval,
                            success_threshold=opt.success_threshold,
                            grad_threshold=opt.densify_grad_threshold,
                            min_opacity=opt.min_opacity,
                            require_purning=False, # Scale-based pruning (모듈(a) 비활성화 / adaptive densification만 적용
