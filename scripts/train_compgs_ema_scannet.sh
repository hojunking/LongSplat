# python train_compgs_ema_revise.py  --eval -s ./data/scannet/scannet_qp37/scene0011_00_qp37/ -m ./outputs/scannet/scene0011_00_qp37/ --mode free --d_mu 0.5 --port 39209 --scene_name scene0011_00 --qp_level qp37 --trust_momentum 0.95
...
OUTPUT_BASE="outputs/free_qp37_com-calc_ours2"
SHEET_NAME="rebuttal"
...
for SCENE in "${SCENES[@]}"; do
  for QP in "${QP_LEVELS[@]}"; do
    SCENE_QP="${SCENE}_${QP}_com-calc_ours2"
    ...
    python train_compgs_ema_revise.py --eval \
        -s ./data/scannet/scannet_${QP}/${SCENE}_${QP}/ \
        -m ./outputs/scannet/${SCENE}_${QP}/ \
        --mode free \
        --d_mu 0.5 \
        --port 39209 \
        --scene_name ${SCENE} \
    ...
    [ $? -ne 0 ] && echo "❌ Training failed for ${SCENE}_${QP}, skipping..." && continue
