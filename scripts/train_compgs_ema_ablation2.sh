...
for SCENE in "${SCENES[@]}"; do
  for QP in "${QP_LEVELS[@]}"; do
    SCENE_QP="${SCENE}_${QP}_compgs_ablation_module2_mom095"
...
for SCENE in "${SCENES[@]}"; do
  for QP in "${QP_LEVELS[@]}"; do
    SCENE_QP="${SCENE}_${QP}_compgs_ablation_module2-3_mom095"
    COMP_PATH="${COMPRESSED_DATA}/${QP}/${SCENE}"
    MODEL_PATH="${OUTPUT_BASE}/${SCENE_QP}"
    ...
    echo ""
    echo "🔵 [1/3] Training with ${QP} images..."
    python train_compgs_ema_ablation2-3.py --eval \
        -s ${COMP_PATH} \
        -m ${MODEL_PATH} \
        -r 2 --mode free \
        --d_mu 0.5  \
