...
class Mast3rMatcher:
    ...
    def _forward(self, img0, img1, intrinsic=None, frame_trust: float = 1.0):
        ...
        # 🔹 (1) Compression-aware confidence scaling
        desc_conf1 *= frame_trust
        desc_conf2 *= frame_trust
        conf1 *= frame_trust
        conf2 *= frame_trust

        # 🔹 (2) Local entropy-based keypoint pruning
        import torch
        import torch.nn.functional as F

        def compute_entropy_map(img):
            """
            img: (3,H,W) tensor
            output: (H,W) entropy map (normalized to [0,1])
            """
            gray = img.mean(0, keepdim=True).unsqueeze(0)  # (1,1,H,W)
            # local 7x7 평균
            unfold = F.unfold(gray, kernel_size=7, padding=3)  # (1,49,H*W)
            probs = unfold.softmax(dim=1)
            entropy = -(probs * probs.log()).sum(dim=1)  # (1, H*W)
            h, w = gray.shape[-2:]
            entropy = entropy.view(1, 1, h, w)
            entropy = F.interpolate(entropy, size=(h, w), mode="bilinear", align_corners=False)
            entropy = entropy.squeeze().clamp(0, 1)
            return entropy


        entropy_map0 = compute_entropy_map(img0[0].cpu())
        entropy_map1 = compute_entropy_map(img1[0].cpu())
        entropy_mask0 = (entropy_map0 > 2.0).float()
        entropy_mask1 = (entropy_map1 > 2.0).float()
        conf1 *= entropy_mask0.to(conf1.device)
        conf2 *= entropy_mask1.to(conf2.device)
